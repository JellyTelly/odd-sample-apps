...
Sub ShowRegistrationExpired()
	Dbg("Registration Expired")
	title = "Registration Expired"
	text  = "Your registration has expired. Please re-register your device"
	ShowErrorDialog(text, title)
End Sub

Sub ShowSubscriptionRequired()
	Dbg("Subscription Required")
	title = "Subscription Required"
	text  = "An updated membership is required to view the requested content. Please navigate to http://oddnetworks.com to update your membership."
	ShowErrorDialog(text, title)
End Sub
...
Sub ShowErrorDialog(text As dynamic, title=invalid as dynamic)
    ...
    if not isstr(title) title = ""
    ShowDialog1Button(title, text, "Ok")
End Sub
